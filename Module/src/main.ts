...
namespace App {
  ...
  listeAliment();
  (document.querySelector("#type")! as HTMLSelectElement).addEventListener(
    "change",
    listeAliment,
  );
  ...
  function listeAliment() {
    const typeAlimentSouhaite = (
      document.querySelector("#type")! as HTMLSelectElement
    ).value;
    let aliments = getListeAlimentType(typeAlimentSouhaite);

    const baliseTbody = document.querySelector(
      "#listeAliments",
    )! as HTMLTableElement;
